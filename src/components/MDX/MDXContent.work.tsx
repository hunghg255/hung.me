...
const components = {
  h1: ({ children, ...props }: React.HTMLAttributes<HTMLHeadingElement>) => (
    <h1 {...props} className='mx-auto mb-8 max-w-3xl scroll-mt-8'>
      {children}
  ...
  h2: ({ children, ...props }: React.HTMLAttributes<HTMLHeadingElement>) => (
    <h2 className='mx-auto mb-5 mt-8 max-w-3xl scroll-mt-8' {...props}>
      {children}
  ...
  h3: ({ children, ...props }: React.HTMLAttributes<HTMLHeadingElement>) => (
    <h3 className='mx-auto mb-5 mt-6 max-w-3xl scroll-mt-8' {...props}>
      {children}
      <svg viewBox='0 0 16 16' height='0.6em' width='0.6em' className='hidden group-hover:block'>
        <g strokeWidth='1.2' fill='none' stroke='currentColor'>
          <path
  ...
  p: ({ ...props }: React.HTMLAttributes<HTMLHeadingElement>) => (
    <p className='paragraph mx-auto mb-4 !max-w-3xl text-natural-800' {...props} />
  ),
  ul: ({ ...props }: React.HTMLAttributes<HTMLUListElement>) => (
    <ul className='mx-auto my-5 max-w-3xl scroll-m-20 pl-8 text-base leading-7' {...props} />
  ),
  ...
  figure: ({ children, ...props }: React.HTMLAttributes<HTMLPreElement> & {}) => {
    ...
    return (
      <figure
        ...
        {...props}
      >
        <button className='sticky left-2 right-2 top-2 mb-[-32px] ml-auto group-hover:flex md:hidden'>
          Copy
        </button>
        {children}
      </figure>
  ...
  figcaption: ({ children, ...props }: React.HTMLAttributes<HTMLPreElement> & {}) => {
          ...
          <button
            onClick={() =>
              navigator.clipboard.writeText(
                copyTextRef.current?.querySelector('span')?.textContent as string,
              )
            }
  ...
  a: ({ className, ...props }: React.ComponentProps<typeof Link>) => (
    <Link
      className={cn('hover:text-primary-hover font-medium text-primary', className)}
      {...props}
    />
  ),
  img: ({ className, alt, ...props }: React.ComponentProps<typeof Image>) => (
    <Image
      className={cn('my-8 w-full rounded-md border border-natural-200', className)}
      width={800}
      height={800}
      alt={alt}
      {...props}
  ...
  // Tabs: ({ className, ...props }: TabsProps) => <Tabs className={'max-w-3xl'} {...props} />,
  // Tab,
  // Files: ({ className, ...props }: FilesProps) => <Files className={'max-w-3xl'} {...props} />,
  // File,
  // CodePreview,
  ComponentPreview,
